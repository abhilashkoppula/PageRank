...
new_pagerank = FOREACH (COGROUP original_data by source , previous_pagerank by source OUTER) GENERATE group as source , (1-$dampingFactor)+$dampingFactor*(SUM(previous_pagerank.pagerank) IS NULL ? 0:SUM(previous_pagerank.pagerank)) as pagerank, FLATTEN(original_data.out) as out;
